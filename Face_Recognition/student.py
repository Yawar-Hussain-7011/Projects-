...
class Student:
    def __init__(self,root):
        ...
        dep_comb = ttk.Combobox(current_course_frame,textvariable=self.var_dep,font=("times new roman",13,"bold"),state = "readonly",width=20)
        dep_comb["values"] = ("Select Department","Computer Science","IT","Civil","Electrical","Eletronic","Mechnical")
        dep_comb.current(0)
        ...
        course_comb = ttk.Combobox(current_course_frame,textvariable=self.var_course,font=("times new roman",13,"bold"),state = "readonly",width=20)
        course_comb["values"] = ("Select Course","B.Tech","BCA","BBA","MBA")
        ...
        year_comb = ttk.Combobox(current_course_frame,textvariable=self.var_year,font=("times new roman",13,"bold"),state = "readonly",width=20)
        year_comb["values"] = ("Select Year","1st year","2nd year","3rd year","4th year")
        year_comb.current(0)   
        ...
        semester_comb = ttk.Combobox(current_course_frame,textvariable=self.var_semester,font=("times new roman",13,"bold"),state = "readonly",width=20)
        semester_comb["values"] = ("Select Semester","Sem-1","Sem-2","Sem-3","Sem-4","Sem-5","Sem-6","Sem-7","Sem-8")
        ...
        div_comb = ttk.Combobox(class_student_frame,textvariable=self.var_div,font=("times new roman",13,"bold"),state = "readonly",width=18)
        div_comb["values"] = ("A","B","C",)
        div_comb.current(0)
        div_comb.grid(row=1,column=1,padx=10,pady=5,sticky=W)
        ...
        gender_comb = ttk.Combobox(class_student_frame,textvariable=self.var_gender,font=("times new roman",13,"bold"),state = "readonly",width=18)
        gender_comb["values"] = ("Select","Female","Male","Other",)
        gender_comb.current(0)
        gender_comb.grid(row=2,column=1,padx=10,pady=5,sticky=W)
        ...
        delet_btn = Button(btn_frame,text = "Delete",command=self.delete_data,width=17,font=("times new roman",13,"bold"),bg = "blue",fg = "white")
        ...
        img_right =Image.open("face_recognition/iStock-1.jpg")
        ...
        search_comb = ttk.Combobox(search_frame,font=("times new roman",13,"bold"),state = "readonly",width=15)
        search_comb["values"] = ("Select","Roll No.","Phone_No.")
        ...
        self.student_table =ttk.Treeview(table_frame,column=("dep","course","year","sem","id","std_name","div","roll","dob","email","gender",
                                                             "phone","address","teacher","photo"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
    ...
    def add_data(self):
        if self.var_dep.get() == "Select Department" or self.var_std_name.get() == "" or self.var_std_id.get()=="":
            ...
            try:
                ...
                conn=mysql.connector.connect(host = "localhost",user = "root", password = "Yawar@701",database = "face_recognition" )
                my_cursor = conn.cursor()
    ...
    def update_data(self):
        if self.var_dep.get()=="Select Department" or self.var_std_name.get()=="" or self.var_std_id.get()=="":
            ...
            try:    
                update = messagebox.askyesno("Update","Do you want to update Student_Details ", parent = self.root)
                if update>0:
                    # conn = pymysql.connect(host = "localhost",user = "root", password = "Yawar@701",database = "face_recognition" )
                    # my_cursor = conn.cursor()
                    conn = mysql.connector.connect(host = "localhost",user = "root", password = "Yawar@701",database = "face_recognition" )
    ...
    def reset_data(self):
        ...
        self.var_std_id.set("Select Student_id"),
        self.var_std_name.set("Select Student_name"),
        self.var_div.set("Select Divsion"),
    ...
    def generate_dataset(self):
        if self.var_dep.get() == "Select Department" or self.var_std_name.get() == " " or self.var_std_id.get() == " ":
            ...
            try:
                ...
                face_classifier = cv2.CascadeClassifier("face_recognition/haarcascade_frontalface_default.xml")
                ...
                while True:
                    ...
                    if face_cropped(my_frame) is not None:
                        ...
                        file_name_path = "face_recognition/data/user."+str(id)+"."+str(img_id)+".jpg"
